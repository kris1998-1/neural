...
def calculate_energy(start_pos, end_pos, start_height, end_height, horizontal_energy_per_m, climb_energy_per_m, descent_energy_per_m, systems_energy_per_m, cell_size=100):
    """
    Универсальный расчет энергетических затрат (Вт·ч) для перемещения с учетом вертикали и горизонтали
    """
    dx = (end_pos[0] - start_pos[0]) * cell_size
    dy = (end_pos[1] - start_pos[1]) * cell_size
    distance = np.sqrt(dx * dx + dy * dy)
    height_diff = end_height - start_height

    horizontal = distance * horizontal_energy_per_m
    vertical = abs(height_diff) * (climb_energy_per_m if height_diff > 0 else descent_energy_per_m)
    systems = distance * systems_energy_per_m
...
class Simulation:
    ...
    def is_enough_energy(self, start_pos, end_pos, start_height, end_height, available_energy):
        """
        Универсальная проверка, хватит ли энергии для перемещения между точками с учетом высоты.
        """
        required = calculate_energy(
            start_pos, end_pos, start_height, end_height,
            self.HORIZONTAL_ENERGY, self.CLIMB_ENERGY, self.DESCENT_ENERGY, self.SYSTEMS_ENERGY, self.cell_size
        )
        return required <= available_energy, required

    def get_point_height(point, stations, station_heights, default_height):
        """
        Возвращает высоту точки: если это станция - возвращает высоту станции, иначе - default_height.
        """
        for i, st in enumerate(stations):
            if np.allclose(point, st, atol=1e-2):
                return station_heights[i]
